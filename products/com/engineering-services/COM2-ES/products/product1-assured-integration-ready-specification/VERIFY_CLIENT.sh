...
echo "=== COM2 PRODUCT1 — CLIENT VERIFICATION ==="
...
grep -Fq 'service_id = "COM2-ES"' PRODUCT.toml || {
...
test -f specification/COM2_ODP_CORRESPONDENCE_SPECIFICATIONS.md
...
test -f uml/information-dynamic-schemas.puml
...
grep -Fq 'canonical_content_files = 22' PRODUCT.toml || {
...
CORR="specification/COM2_ODP_CORRESPONDENCE_SPECIFICATIONS.md"
SPO="assurance/SPECIFICATION_PROOF_OBLIGATIONS.md"

CS_COUNT="$(
  grep -Ec '^# CS-COM2-' "$CORR" || true
)"

CR_COUNT="$(
  grep -Ec '^## CR-COM2-' "$CORR" || true
)"

CL_COUNT="$(
  grep -Ec '^### CL-COM2-' "$CORR" || true
)"

SPO_COUNT="$(
  grep -Ec '^### SPO-COM2-' "$SPO" || true
)"

SAT_COUNT="$(
  grep -Ec '^Disposition: `SATISFIED`$' "$SPO" || true
)"
...
test "$CL_COUNT" = "35" || {
  echo "FAIL: expected 35 Correspondence Links" >&2
  exit 14
}

test "$SPO_COUNT" = "4" || {
  echo "FAIL: expected 4 Specification Proof Obligations" >&2
  exit 15
}

test "$SAT_COUNT" = "4" || {
  echo "FAIL: expected 4 SATISFIED SPO dispositions" >&2
  exit 16
}

echo
echo "PASS: COM2 Product1 assurance invariants = 6 CS / 6 CR / 35 CL / 4 SPO SATISFIED"

echo
echo "PASS: COM2 Product1 1.0.0 complete and intact"
